...
def procesar_linea(linea, inquilinos, deudas):
    ...
    if '*' in partes:  # Agregar nuevo inquilino
        ...
        if '~' in partes:  # Repartir deuda entre inquilinos
            ...
            if pagan:  # Verificar que haya inquilinos que deban pagar
            ...
            if involucrados:  # Verificar que haya inquilinos involucrados
...
def main():
    ...
    #archivo_corto = "transacciones_simple.txt"
    
    inquilinos = set()
    deudas = defaultdict(float)
    
    for archivo in [archivo_largo]: #agregar archivos_corto
